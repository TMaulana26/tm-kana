...
<template>
  <Dialog v-model:open="isOpen">
    <DialogContent
      ...
      class="max-w-md w-[92vw] bg-[#f4f3ec] dark:bg-slate-900 border-[3px] border-slate-950 dark:border-slate-700 rounded-none p-6 shadow-[6px_6px_0px_0px_#000] dark:shadow-[6px_6px_0px_0px_#ef4444]"
    >
      <DialogHeader class="border-b-[3px] border-slate-950 dark:border-slate-700 pb-4 mb-5 relative">
        <div class="flex items-start gap-3.5 pr-10">
          <div
            class="w-10 h-10 bg-amber-300 dark:bg-amber-400 border-[2px] border-slate-950 dark:border-slate-700 flex items-center justify-center shrink-0 shadow-[2px_2px_0px_0px_#000] dark:shadow-[2px_2px_0px_0px_#ef4444] mt-0.5"
          >
      ...
      <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-2">
        ...
        <NeoBrutalistButton
          ...
          variant="danger"
          @click="handleConfirm"
          class="w-full sm:w-auto px-5 py-2.5 bg-rose-400 hover:bg-rose-500 text-black text-xs sm:text-sm font-black"
        >
          {{ $t("practice.leaveBtn") }}
        </NeoBrutalistButton>
      </div>
