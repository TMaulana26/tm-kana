...
Palet warna TM-KANA dirancang dengan rasio kontras tinggi dan makna fungsi yang spesifik:
...
- **Dark Mode Canvas (`#121318`):** Hitam arang mendalam (*deep charcoal ink*). Memberikan kontras tegas terhadap siluet putih tanpa membuat mata lelah.
- **Card Surface (`#ffffff` light / `#0f172a` dark):** Latar kartu konten utama.

### B. Functional & Accent Colors
- **Amber (`#fbbf24` / `#f59e0b`):** Warna primer aksi utama, sorotan kartu profil, kartu CTA kuis, dan tombol bersihkan kanvas.
...
- **Batas Tepi Berkarakter:** Seluruh pembatas menggunakan border tebal:
  - Komponen atomik (tombol, input, kartu): `border-[2px]` hingga `border-[3px] border-slate-950 dark:border-white`.
  - Struktur besar (sidebar, header, modal): `border-[4px] border-slate-950 dark:border-white`.
- **Grid Responsif:**
...
- **Kartu Besar / Dialog Modal:**
  ...
  - Dark Mode: `shadow-[6px_6px_0px_0px_#fff]`
- **Tombol Standar & Card Biasa:**
  - Light Mode: `shadow-[3px_3px_0px_0px_#000]` atau `shadow-[4px_4px_0px_0px_#000]`
  - Dark Mode: `shadow-[3px_3px_0px_0px_#fff]` atau `shadow-[4px_4px_0px_0px_#fff]`
- **Micro Badge / Mini Buttons:**
  - `shadow-[2px_2px_0px_0px_#000] dark:shadow-[2px_2px_0px_0px_#fff]`
- **Efek Tekan (Tactile Press Interaction):**
  - Pada status `:active`: Elemen bergeser `translate-x-[2px] translate-y-[2px]` dan ukuran bayangan menyusut menjadi `shadow-[1px_1px_0px_0px_#000]`, memberikan sensasi tombol fisik mekanis yang nyata.
...
- Tombol dengan kontras warna berani, border tebal hitam/putih, dan bayangan tegas.
...
### Do's:
- Gunakan selalu border tebal `border-slate-950 dark:border-white` pada setiap kontainer.
- Gunakan bayangan solid `shadow-[Xpx_Xpx_0px_0px_#000]` tanpa blur.
