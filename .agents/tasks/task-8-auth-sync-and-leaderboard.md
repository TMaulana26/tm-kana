...
### [ ] Persiapan Backend Laravel di VPS & Database
- [ ] Inisiasi proyek Laravel 11 API di VPS dengan PHP 8.2+.
- [ ] Pasang dan konfigurasikan `laravel/sanctum` (Token/Cookie SPA) dan `laravel/socialite` (Google Login).
- [ ] Buat migrasi tabel Eloquent: `users`, `user_progress`, dan `leaderboard_scores`.
- [ ] Konfigurasikan CORS di `config/cors.php` dan Cloudflare `TrustProxies`.
...
- [ ] Buat endpoint Laravel: `POST /api/auth/google`, `GET /api/user/progress`, `POST /api/user/sync`.
...
- [ ] Buat endpoint Laravel: `GET /api/leaderboard` dan `POST /api/leaderboard/submit` (dengan validasi anti-cheat).

### [ ] Internasionalisasi & Type-Safety
- [ ] Tambahkan kamus i18n untuk namespace `auth` dan `leaderboard` pada `id.json`, `en.json`, dan `ja.json`.
- [ ] Buat interface TypeScript lengkap di `src/types/auth.ts` dan `src/types/leaderboard.ts`.

### [ ] Pengujian Unit & E2E
- [ ] Buat unit test `tests/utils/progressSync.spec.ts` untuk memvalidasi algoritma penggabungan data offline & online.
...
- [ ] `npm run test` (Semua skenario tes unit lulus 100%)
- [ ] `npm run build` (Kompilasi produksi bersih)
